...
public class Exe5GroupSumClump {
 ...
 */
    ...
    public boolean groupSumClump(int start, int[] nums, int target) {
        ...
        int offset = 1;
        for(offset=1; offset<nums.length-start; offset++) {
            if(nums[start]!=nums[start+offset]) break;
        }
        return groupSumClump(start+offset, nums, target) ||
                groupSumClump(start+offset, nums, target-nums[start]*offset);
    }
